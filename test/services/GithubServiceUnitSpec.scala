  ...
  "GithubService .getFileContents" should {
    "get the contents of a file and return it as a string" in {
      (mockGithubConnector.getFileContents[String](_: String, _: String, _: String)(_: OFormat[String], _: ExecutionContext))
        ...
        .returning(Future(Right(encodedFileContents)))
        .once()
      whenReady(testGithubService.getFileContents("jake-raffe", "Back-end_Project", ".gitignore")) { result =>
        result shouldBe Right(fileContents)
      }
    }
  }

  "GithubService .getExistingFileForUpdating" should {
    "find a file from the repository and return its name, decoded content and sha code" in {
      (mockGithubConnector.getFileNameContentsSha(_: String, _: String, _: String)(_: OFormat[String], _: ExecutionContext))
        .expects("jake-raffe", "Back-end_Project", ".gitignore",*,*)
        .returning(Future(Right(ExistingFile(".gitignore", encodedFileContents, "bcbcbcjdjdjd"))))
        .once()
